...
import com.silvermoon.fabui.R;

public class AnimFrag extends BottomDialogFrag {
    ...
    private BottomViewBehaviour mBottomSheetBehavior;
    ...
    private BottomViewBehaviour.BottomSheetCallback mBottomSheetBehaviorCallback = new BottomViewBehaviour.BottomSheetCallback() {

        @Override
        public void onStateChanged(View bottomSheet, int newState) {
            switch (newState) {
                case BottomViewBehaviour.STATE_HIDDEN:
                    if (callbacks != null) {
                        callbacks.onResult("swiped_down");
                    }
                    dismiss();
                    break;
                case BottomViewBehaviour.STATE_COLLAPSED:
                    ViewGroup.LayoutParams params = view_main.getLayoutParams();
                    params.height = ViewGroup.LayoutParams.MATCH_PARENT;
                    view_main.setLayoutParams(params);
                    break;
                case BottomViewBehaviour.STATE_EXPANDED:
    ...
    public void setupDialog(Dialog dialog, int style) {
        ...
        if (viewPager != null) {
            BottomViewUtils.setupViewPager(viewPager);
        }
        ...
        mBottomSheetBehavior = BottomViewBehaviour.from(((View) contentView.getParent()));
        ...
        dialog.setOnShowListener(new DialogInterface.OnShowListener() {
            ...
            public void onShow(DialogInterface dialog) {
                ...
                BottomViewBehaviour.from(bottomSheet).setState(BottomViewBehaviour.STATE_COLLAPSED);
                if (viewgroup_static != null) {
                    int range = (int) (metrics.heightPixels - (metrics.density * peek_height) - getStatusBarHeight(getContext()));
        ...
        if (behavior != null && behavior instanceof BottomViewBehaviour) {
            ((BottomViewBehaviour) behavior).setBottomSheetCallback(mBottomSheetBehaviorCallback);
        }
    ...
    private void fabAnim() {
        ...
        ArcAnimation anim = new ArcAnimation(0, metrics.widthPixels / 2 - fab_pos_x - (fab_size / 2), 0, -(metrics.density * ((peek_height / 2) - ((((metrics.heightPixels - fab_pos_y) - fab_size) / metrics.density)))));
    ...
    public void closeFilter(final Object o) {
        ...
        if (BottomViewBehaviour.from(bottomSheet).getState() == BottomViewBehaviour.STATE_EXPANDED) {
            BottomViewBehaviour.from(bottomSheet).setState(BottomViewBehaviour.STATE_COLLAPSED);
        }
        fabulous_fab.setVisibility(View.VISIBLE);
        view_main.setVisibility(View.INVISIBLE);
...
//                            fabulous_fab.setY(fab_outside_y_offest - fab_pos_y + getStatusBarHeight(getContext()));
                        ...
                        ArcAnimation anim = new ArcAnimation(0, -(metrics.widthPixels / 2 - fab_pos_x - (fab_size / 2)), from_y, to_y);
                        anim.setDuration(anim_duration);
                        fl.startAnimation(anim);
                        anim.setAnimationListener(new Animation.AnimationListener() {
                            @Override
